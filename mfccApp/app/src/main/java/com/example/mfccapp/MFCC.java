...
public class MFCC {
	...
	private final static int       n_mfcc       		= 20; //13
	private final static double    fMin                 = 0.0;
	private final static int       n_fft                = 2048; //1764
	private final static int       hop_length           = 2048; //40 //512
	private final static int	   n_mels               = 64; //64

	private final static double    sampleRate           = 51200.0; //44100
...
//mel spectrogram, librosa
	public double[][] melSpectrogram(double[] y){
		...
		Log.v(LOG_TAG, "The code reached melSpectrogram end successfully");
		Log.v(LOG_TAG, "size returned is : " + melS.length + " " + melS[0].length);
...
//stft, librosa
	private double[][] stftMagSpec(double[] y){
		...
		Log.v(LOG_TAG, "The code reached stftMagSpec end successfully");
		Log.v(LOG_TAG, "size returned is : " + fftmagSpec.length+ " " + fftmagSpec[0].length);
		return fftmagSpec;
	}

	private double[] magSpectrogram(double[] frame){
		Log.v(LOG_TAG, "The code entered magSpectrogram successfully");
//		Log.v(LOG_TAG, "size returned is : " + freqs.length);
		double[] magSpec = new double[frame.length];


		fft.process(frame);
		Log.v(LOG_TAG,"exited fft process");
		for (int m = 0; m < frame.length; m++) {
			magSpec[m] = fft.real[m] * fft.real[m] + fft.imag[m] * fft.imag[m];
		}
		Log.v(LOG_TAG, "The code reached magSpectrogram end successfully");
		Log.v(LOG_TAG, "size returned is : " + magSpec.length);
		return magSpec;
	}
...
//get hann window, librosa
	private double[] getWindow(){
		...
		Log.v(LOG_TAG, "The code reached getWindow end successfully");
		Log.v(LOG_TAG, "size returned is : " + win.length);
		return win;
	}
...
//frame, librosa
	private double[][] yFrame(double[] ypad){
		...
		Log.v(LOG_TAG, "The code reached yFrame end successfully");
		Log.v(LOG_TAG, "size returned is : " + winFrames.length + " " + winFrames[0].length);
		return winFrames;
	}
...
//mel, librosa
	private double[][] melFilter(){
		...
		Log.v(LOG_TAG, "The code reached melFilter end successfully");
		Log.v(LOG_TAG, "size returned is : " + weights.length + " " + weights[0].length);
		return weights;
...
//fft frequencies, librosa
	private double[] fftFreq() {
		...
		Log.v(LOG_TAG, "The code reached fftFreq end successfully");
		Log.v(LOG_TAG, "size returned is : " + freqs.length);
		return freqs;
	}
...
//mel frequencies, librosa
    ...
    final double[] melFHigh   = freqToMel(HighFFreq);
		...
		mels = melToFreq(mels);
		Log.v(LOG_TAG, "The code reached melFreq end successfully");
		Log.v(LOG_TAG, "size returned is : " + mels.length);
		return mels;
	}
